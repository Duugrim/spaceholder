...
**Сравнение с массивами:** для массивов (например, `system.health.injuries`) полная замена работает: `await this.update({ 'system.health.injuries': filtered })` — массив подменяется целиком. Для объектов-словарей полагаться на замену целиком не стоит — использовать `-=key` для удаления.

Итого: чтобы запись в объекте документа действительно исчезла, использовать ключ вида **`path.-=keyName`** со значением **`null`** в `update()`.
