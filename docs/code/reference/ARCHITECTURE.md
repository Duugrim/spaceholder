...
doc-type: reference
status: verify
note: "Дата в тексте 2025-10; сверить compatibility Foundry с system.json (v14+)."
tags:
...
- `module/anatomy-manager.mjs`:
  ...
  - `createActorAnatomy(id, { healthMultiplier, overrides })` — готовит копию для актёра, нормализует слоты (`slotRef`), `relations` и производные `links` (см. `docs/code/reference/ANATOMY_SYSTEM.md`).
...
## Данные и ресурсы
- Анатомии (рантайм): `data/anatomy/*.json` + зеркало в `module/data/anatomy/` в репозитории; подробности — `docs/code/reference/ANATOMY_SYSTEM.md`.
