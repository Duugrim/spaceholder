/**
 * LEGACY / DEPRECATED (2026-01-24)
 *
 * Ранее этот файл реализовывал кастомные режимы видимости токенов для актёров типа `globalobject`:
 * - public
 * - halfHidden (legacy alias: hidden)
 * - fullHidden
 * - secret
 *
 * От этой механики отказались в пользу core Foundry (свет/зрение), потому что она не гарантирует
 * корректный ре-рендер/пересчёт видимости при нестандартных перемещениях и изменениях «источников влияния».
 * В частности, проявлялся баг с отсутствием ре-рендеринга токенов при ситуациях, где
 * «источник влияния от токена, а не токен от влияния», из-за чего состояние видимости могло рассинхронизироваться.
 *
 * Файл оставлен для истории и дальнейшей миграции (можно перенести в legacy/архив).
 */
