...
- `docs/` — техническая документация: `SHOOTING_SYSTEM.md` (стрельба/payload/сегменты), анатомия, влияние, глобальная карта и т.д. **При доработках интерфейса см. `docs/UI_DESIGN_GUIDE.md`** — правила и эталоны дизайна. **При проблемах с листами App V2 (кнопки не срабатывают, вкладка сбрасывается, удаление записей в документе не применяется) см. `docs/APP_V2_SHEET_PATTERNS.md`** — проверенные паттерны и решения.
