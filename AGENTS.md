...
- `docs/` — техническая документация: `SHOOTING_SYSTEM.md` (стрельба/payload/сегменты), анатомия, влияние, глобальная карта и т.д. **При доработках интерфейса см. `docs/UI_DESIGN_GUIDE.md`** — правила и эталоны дизайна. **При проблемах с листами App V2 (кнопки не срабатывают, вкладка сбрасывается, удаление записей в документе не применяется) см. `docs/APP_V2_SHEET_PATTERNS.md`** — проверенные паттерны и решения. Для canvas-drop подсистемы item piles см. `docs/ITEM_PILES_SH.md`.
- `pack-src/` — исходные JSON встроенных компендиумов (в git). Каталоги **`packs/<имя-пака>/`** — LevelDB, собираемые из `pack-src/`; не класть туда «ручные» JSON как единственный источник записей. Рабочий процесс и команды: **`docs/COMPENDIUM_PACKS.md`**.
...
Компендиумы (после правок в `pack-src/`): закрыть Foundry, затем `npm run pack:sh-test-items` (см. `docs/COMPENDIUM_PACKS.md`).
...
- **Сначала сверяться с `docs/APP_V2_SHEET_PATTERNS.md`**: там описаны проверенные решения (кнопки, сохранение вкладки, удаление ключей в документе).
- Если после дебага проблемы вида «кнопка должна нажиматься, а не нажимается», «вкладка сбрасывается», «удаление не сохраняется» и т.п. найдено решение, которого **нет** в этой доке — **предложить пользователю добавить его в `docs/APP_V2_SHEET_PATTERNS.md`**, чтобы нарабатывать шпаргалку под специфику проекта.
